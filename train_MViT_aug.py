...
class YourVideoDataset(Dataset):
    def __init__(self, root_dir, transform=None, frames_per_clip=None,val=None):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.val:
                ...
                if total_frames < T:
                    # Pad short videos (unchanged behavior)
                    base = np.arange(total_frames)
                    pad = T - total_frames
                    frame_indices = np.concatenate([base, np.full((pad,), base[-1], dtype=int)])

                else:
                    # -------- TEMPORAL AUGMENTATION (TRAINING) --------
                    
                    # 1️⃣ Random stride
                    stride = np.random.choice([1, 2, 3])

                    # Maximum valid start index
                    max_start = total_frames - stride * T
                    if max_start < 0:
                        max_start = 0

                    # 2️⃣ Random clip start
                    start = np.random.randint(0, max_start + 1)

                    frame_indices = start + stride * np.arange(T)

                    # 3️⃣ Temporal jitter (drop / duplicate)
                    if np.random.rand() < 0.3:
                        jitter_idx = np.random.randint(0, T)
                        frame_indices[jitter_idx] = frame_indices[max(jitter_idx - 1, 0)]
...
if __name__ == "__main__":
    ...
    train_dataset = YourVideoDataset(train_datapath, transform=transform, frames_per_clip=16, val=False)
    val_dataset = YourVideoDataset(val_datapath, transform=transform, frames_per_clip=16, val=True)
